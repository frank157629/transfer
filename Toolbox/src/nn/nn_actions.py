...
class NeuralNetworkActions():
    ...
    def calculate_from_ode(self, output):
        ...
        if self.cfg.modelling_method:
            ...
            y_processed = self.modelling_full.odequation_hl(0, output.split(split_size=1, dim=1))
        else:
    ...
    def compute_ntk_jacrev(self, x):
        ...
        y_processed = self.modelling_full.odequation_hl(0, y.split(split_size=1, dim=1))
        """
    ...
    def pinn_train2(self, num_of_skip_data_points, num_of_skip_col_points, num_of_skip_val_points, wandb_run=None):
        ...
        print("\nNumber of labeled training data:", x_train.shape[0],
        ...
        total_test_loss =  self.test_model(0,10,wandb_run)
        # total_test_loss =  self.test_model(0,500,wandb_run)
    ...
    #     # total_traj = 5
    #     total_traj = 2
    #     num_per_traj = int(self.data_loader.sample_per_traj) #1000
    #     print(f"[DEBUG] samples / traj = {num_per_traj}")
    #     print("x_test time head:", x_test[:20, 0])
    ...
    #             plt.plot(x_test[down_limit + num_per_traj*traj:down_limit+num_per_traj*(traj+1),0].detach().cpu().numpy(), output[down_limit + num_per_traj*traj:down_limit+num_per_traj*(traj+1),var].detach().cpu().numpy(), label="Predicted")
    #             plt.plot(x_test[down_limit + num_per_traj*traj:down_limit+num_per_traj*(traj+1),0].detach().cpu().numpy(), target[down_limit + num_per_traj*traj:down_limit+num_per_traj*(traj+1),var].detach().cpu().numpy(), label="True")
    ...
    def log_plot(self, output, target, epoch, run, x_test):
        ...
        for var in range(len(self.keys)):
            ...
            plt.title(f"Trajectories {starting_traj} to {starting_traj + total_traj} for variable {self.keys[var]}")
            # plot with x axis x_test[:,0]
            plt.plot(x_test[down_limit:upper_limit, 0].detach().cpu().numpy(),
                     output[down_limit:upper_limit, var].detach().cpu().numpy(), label="Predicted")
            plt.plot(x_test[down_limit:upper_limit, 0].detach().cpu().numpy(),
                     target[down_limit:upper_limit, var].detach().cpu().numpy(), label="True")
            # plt.plot(x_test[down_limit:upper_limit, 0].detach().cpu().numpy(),output[down_limit:upper_limit, var].detach().cpu().numpy(), label="Predicted")
            # plt.plot(x_test[down_limit:upper_limit, 0].detach().cpu().numpy(),target[down_limit:upper_limit, var].detach().cpu().numpy(), label="True")
            plt.xlabel("Time(s)")
            plt.legend()
