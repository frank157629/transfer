...
"""
convergence1.py
挑选 δ 收敛到 k·2π+Δ₀ 的轨迹，并按三类（0、其它 2kπ、非收敛）采样-保存
"""
...
sampling_ratio = dict(region_0 = 1,      # δ → 0+Δ₀
                      ...
                      non_converged=0)
num_points_to_save = 30              # Total trajectories
# =========================
...
def plot_ic_scatter(groups, data):
    """初始点散点图：region_0 绿色；其它 2kπ 蓝色；非收敛 红色"""
...
def quick_plot(traj_ids, title):
    ...
    for idx in traj_ids:
        t = np.arange(data.shape[2]) / 1000.0          # 假设 1 kHz 采样
