...
class ODE_modelling():
    ...
    def check_ic_yaml(self,init_conditions):
        ...
        if self.theme == "SM":
            ...
            modeling_guide_path = os.path.join(self.init_conditions_dir, "modellings_guide_gfl.yaml")
            modeling_guide = OmegaConf.load(modeling_guide_path)
        elif self.theme == "GFM":
    ...
    def create_init_conditions_set3(self):
        ...
        #Haitian, to debug the shape of init_cond_table
        rows = len(init_condition_table)
        cols = len(init_condition_table[0]) if rows > 0 else 0
    ...
    #Haitian, originally defined as solve_sm_model, definition remains wholely the same, able to be directly used for solving ODEs of other model.
