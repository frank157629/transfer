...
def plotting_solution(solution,flag=True):
    ...
    plt.title('GFL_2nd_order second-order')
    plt.xlabel('Time (s)')
...
# Plotting the solution with gridspec
def plotting_solution_gridspec_original_all_7th(sol, model, show=True):
    # check if sol[0] exists
    ...
    if 'message' in sol:
        ...
        axs[0].plot(solution.t, solution.y[0], label='γ')
        axs[1].plot(solution.t, solution.y[1], label='δ')
        axs[2].plot(solution.t, solution.y[2], label='θ_{GFL_2nd_order}')
        axs[3].plot(solution.t, solution.y[3], label='i_{gd}^{g}')
        axs[4].plot(solution.t, solution.y[4], label='i_{gq}^{g}')
        axs[5].plot(solution.t, solution.y[5], label='v_{od}^{g}')
        axs[6].plot(solution.t, solution.y[6], label='v_{oq}^{g}')
        ...
        for i in range(len(sol)):
            ...
            γ, δ, θ0, i_d, i_q, v_od, v_oq = solution.y[:, 0]  # γ, δ, θ_PLL, i_gd, i_gq, v_od, v_oq
            ic_str = (f"γ={γ:+.3f}, δ={δ:+.3f}, θ₀={θ0:+.2f}, "
                      f"i_d={i_d:+.2f}, i_q={i_q:+.2f}, "
                      f"v_od={v_od:+.2f}, v_oq={v_oq:+.2f}")
            print(ic_str)
            axs[0].set_title(f"7th_Order_model:" )
            axs[0].set_ylabel('γ')
            axs[0].grid(True)
            axs[1].set_xlabel('Time (s)')
            axs[1].set_ylabel('δ')
            axs[1].grid(True)
            axs[2].set_xlabel('Time (s)')
            axs[2].set_ylabel('θ_{GFL_2nd_order}')
            axs[2].grid(True)
            axs[3].set_xlabel('Time (s)')
            axs[3].set_ylabel('i_{gd}^{g}')
            axs[3].grid(True)
            axs[4].set_xlabel('Time (s)')
            axs[4].set_ylabel('i_{gq}^{g}')
            axs[4].grid(True)
            axs[5].set_xlabel('Time (s)')
            axs[5].set_ylabel('v_{od}^{g}')
            axs[5].grid(True)
            axs[6].set_xlabel('Time (s)')
            axs[6].set_ylabel('v_{oq}^{g}')
            axs[6].grid(True)
            for ax in axs:
                ax.legend()
    if show:
