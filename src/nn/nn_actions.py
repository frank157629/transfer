...
from src.nn.nn_model import Net, Network, FullyConnectedResNet, Kalm
...
class NeuralNetworkActions():
    ...
    def __init__(self, cfg, modelling_full):  # The modelling equations are used, must be predefined, more choices to be added such as dynamic modelling
        ...
        self.GFM = GFM(self.cfg)
        self.model = self.model.to(self.device)
        self.early_stopping = EarlyStopping(patience=cfg.nn.early_stopping_patience, verbose=True,
                                            delta=cfg.nn.early_stopping_min_delta)
    ...
    def setup_nn(self):
        ...
        self.model = self.model.to(self.device)
        self.early_stopping = EarlyStopping(patience=self.cfg.nn.early_stopping_patience, verbose=True,
                                            delta=self.cfg.nn.early_stopping_min_delta)
    ...
    def define_nn_model(self):
        ...
        if self.cfg.nn.type == "KAN":  # Static architecture of the neural network
            ...
            model = FullyConnectedResNet(self.input_dim, self.cfg.nn.hidden_dim, self.output_dim, num_blocks,
    ...
    def custom_learning_rate(self, lr_name):  # Choose between "StepLR", "MultiStepLR", "ExponentialLR", "ReduceLROnPlateau
        ...
        if lr_name == 'StepLR':
            ...
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=0.1, patience=10,
                                                             verbose=True)
    ...
    def update_loss_weights(self, old_weight_data, old_weight_dt, old_weight_pinn, old_weight_pinn_ic, loss_data,
                            loss_dt, loss_pinn, loss_pinn_ic, epoch):
        if self.cfg.nn.weighting.update_weight_method == "Static":
            ...
            result = self.relobralo_loss(loss_data, loss_dt, loss_pinn, loss_pinn_ic, old_weight_data, old_weight_dt,
                                         old_weight_pinn, old_weight_pinn_ic)
            ...
            if epoch % self.update_weights_freq == 0:
                with torch.no_grad():
                    if self.wandb_run is not None:
                        ...
                        log_data.update(
                            {f"soft_attention_weights grad{i}": soft_attention_weights.grad[i] for i in range(4)})
    ...
    def initialize_loss_weights(self, weight_data, weight_dt, weight_pinn, weight_pinn_ic):
        if self.cfg.nn.weighting.update_weight_method == "Sam":
            self.soft_attention_weights = torch.nn.Parameter(
                torch.tensor([weight_data, weight_dt, weight_pinn, weight_pinn_ic], dtype=torch.float32,
                             requires_grad=True))
    ...
    def pinn_train2(self, num_of_skip_data_points=None, num_of_skip_col_points=None, num_of_skip_val_points=None, wandb_run=None):
        ...
        ...
        x_train, y_train, x_train_col, x_train_col_ic, y_train_col_ic, x_val, y_val = self.data_loader.define_train_val_data2(
            self.cfg.dataset.perc_of_data_points, self.cfg.dataset.perc_of_col_points, num_of_skip_data_points,
            num_of_skip_col_points, num_of_skip_val_points)  # define the training and validation data
        # Create DataLoaders for batch processing
        batch_size = self.cfg.nn.batch_size if self.cfg.nn.batch_size != "None" else max(len(x_train), len(x_train_col),
                                                                                         len(x_train_col_ic))
        ...
        for epoch in range(self.cfg.nn.num_epochs):
            ...
            if self.cfg.nn.batch_size != "None":
                ...
                for (x_batch, y_batch), x_col_batch, (x_ic_batch, y_ic_batch) in zip(train_loader, collocation_loader,
                                                                                     col_ic_loader):
                    ...
                    def closure():
                        ...
                        self.weighting_scheme.log_losses(
                            [loss_total, loss_data, mean_loss_dt, mean_loss_pinn, loss_pinn_ic], epoch,
                            ["total_loss", "data", "dt", "pinn", "pinn_ic"])
            ...
            if (epoch + 1) % 50 == 0:
                print(
                    f'Epoch [{epoch + 1}/{self.cfg.nn.num_epochs}], Loss: {self.loss_total.item():.4f}, Loss_data: {self.loss_data.item():.4f}, Loss_dt: {self.loss_dt.item():.4f}, Loss_pinn: {self.loss_pinn.item():.4f} , Loss_pinn_ic : {self.loss_pinn_ic.item():.4f}',
        ...
        if self.early_stopping.early_stop == True or (epoch + 1) % save_iteration != 0:
            name = f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch + 1}_{self.data_loader.training_shape}_{self.data_loader.training_col_shape}_{self.data_loader.validation_shape}_{self.cfg.dataset.transform_input}_{self.cfg.dataset.transform_output}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}.pth"
            self.save_model(os.path.join(folder_name, name))
        self.final_name = os.path.join(folder_name,
                                       f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch + 1}_{self.data_loader.training_shape}_{self.data_loader.training_col_shape}_{self.data_loader.validation_shape}_{self.cfg.dataset.transform_input}_{self.cfg.dataset.transform_output}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}")
    ...
    def log_plot(self, output, target, epoch, run, x_test, type, starting_traj=0, total_traj=1):
        # log in wandb
        if self.cfg.model.model_flag == "SM":
        ...
        elif self.cfg.model.model_flag == "GFM":
        ...
        for r, k in enumerate(blk_idx):
            ...
            vf_d_true = target[lo:hi, 2].detach().cpu().numpy()
            vf_q_true = target[lo:hi, 3].detach().cpu().numpy()
            vf_d_pred = output[lo:hi, 2].detach().cpu().numpy()
            vf_q_pred = output[lo:hi, 3].detach().cpu().numpy()

            # Ordino i dati in base a t
            sort_idx = np.argsort(t)
            t = t[sort_idx]
            vf_d_true = vf_d_true[sort_idx]
            vf_q_true = vf_q_true[sort_idx]
            vf_d_pred = vf_d_pred[sort_idx]
            vf_q_pred = vf_q_pred[sort_idx]
            ...
            if self.keys[2] == "vfd":
                ...
                axd.plot(t, vf_d_true, color='tab:blue', lw=1.2, label='True' if r == 0 else None)
                axd.plot(t, vf_d_pred, color='tab:orange', lw=1.2, ls='--', label='Pred' if r == 0 else None)
                axd.grid(ls='--', alpha=.3)

            if self.keys[3] == "vfq":
                axf = axes[r, 1]
                axf.set_visible(True)
                axf.plot(t, vf_q_true, color='tab:blue', lw=1.2, label='True' if r == 0 else None)
                axf.plot(t, vf_q_pred, color='tab:orange', lw=1.2, ls='--', label='Pred' if r == 0 else None)
                axf.grid(ls='--', alpha=.3)
        ...
        # 统一标题 / x 轴
        axes[0, 0].set_title('vfd')
        axes[0, 1].set_title('vfq')
    ...
    def loss_over_time(self, x_test, y_test, y_pred, run=None):
        ...
        for value in unique_values:  # for each time step
            ...
            for i in range(y_pred_.shape[1]):
                mae_var.append(
                    self.criterion_mae(y_pred_[:, i], y_true[:, i]).item())  # calculate the mae for each variable
                rmse_var.append(
                    self.criterion(y_pred_[:, i], y_true[:, i]).item())  # calculate the rmse for each variable
            mae.append((mae_var))
            rmse.append((rmse_var))
        mae = np.array(mae)
        rmse = np.array(rmse)
        mae2 = torch.abs(y_test - y_pred)  # Calculate absolute errors for each prediction
        if self.cfg.theme == "GFM":
            var_name = ["xi_d", "xi_q", "vfd", "vfq", "ifd", "ifq", "itd", "itq", "sigma_d", "sigma_q", "gamma_q", "theta_gfm", "theta_grid"]
        else:
        ...
        if run is not None:
            for i in range(y_pred_.shape[1]):
                ...
                plt.close()
                """
                max_mae = torch.max(mae2[:,
                                    i])  # Find the maximum absolute error # calculate the absolute error for each prediction, to find max mae
    ...
    def load_model(self, name=None):
        ...
        if not os.path.exists(model_dir) or len(os.listdir(model_dir)) == 0:
            raise Exception(
                "No model found in the model_dir, please correct path and name of the model from the bucket first")
        if name is None:
    ...
    def plot(self, x_train, y_train, var=0):
        ...
        # Estrai i dati da torch -> numpy
        x = x_train[:, 0].cpu().detach().numpy()
        y = y_train[:, var].cpu().detach().numpy()
        y_hat = y_pred[:, var].cpu().detach().numpy()

        # Ordina i dati in base al tempo (x)
        ...
        plt.plot(x, y_hat, 'kx-', label='Fitted line')
        plt.legend()
        plt.show()
        return
    ...
    def plot_all(self, x_train, y_train):
        ...
        for i in range(y.shape[1]):
            ...
            plt.plot(x, y[:, i], 'ro', label='Original data')
            plt.plot(x, y_hat[:, i], 'kx-', label='Fitted line')
            plt.legend()
            if i % 2 != 0:
                plt.show()
        return
...
def plot_all_dt(self, x_train, y_train):
    ...
    x = x_train[:, 0].cpu().detach().numpy()
    dt = self.calculate_from_ode(y_train).cpu().detach().numpy()
    dt_hat = self.calculate_from_ode(y_pred).cpu().detach().numpy()


    plt.figure(figsize=(10, 5))
    for i in range(dt.shape[1]):
        plt.subplot(1, 2, i % 2 + 1)
        plt.plot(x, dt[:, i], 'ro', label='Original derivative')
        plt.plot(x, dt_hat[:, i], 'kx-', label='Fitted derivative')
        plt.legend()
        if i % 2 != 0:
            plt.show()
    return
