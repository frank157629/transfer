...
class ODE_modelling():
    ...
    def create_init_conditions_set3(self):
        """
        Define the various initial conditions of the synchronous machine (or GFL_2nd_order model) and return a matrix with all the possible combinations.

        Returns:
            list: A matrix with all the possible combinations of initial conditions.
        """
        if self.torch:# if using torch then use the nn_init_cond1.yaml file to create collocation points init conditions
    ...
    def solve(self, x0, method, modelling_full):
        ...
        if method:
            ...
            solution = solve_ivp(modelling_full.odequation_hl, self.t_span, x0, t_eval=self.t_eval)
        else:
